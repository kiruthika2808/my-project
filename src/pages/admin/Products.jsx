...
export default function Products() {
  ...
  const handleFiles = (files) => {
    ...
    setField("images", [...(form.images || []), ...images]);
  };

  const saveProduct = (event) => {
    event.preventDefault();
    if (form.id) updateProduct(form);
    else addProduct({ ...form, image: form.images[0] || "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?auto=format&fit=crop&w=1100&q=85" });
    setEditing(null);
  };
      ...
      {editing && (
        <Card className="mb-6">
          <form className="grid gap-4 lg:grid-cols-2" onSubmit={saveProduct}>
            ...
            <label className="admin-input cursor-pointer lg:col-span-2">Upload multiple images
              <input className="hidden" type="file" multiple accept="image/*" onChange={(event) => handleFiles(event.target.files)} />
            </label>
